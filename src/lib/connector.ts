...
export class Connector {
    ...
    public constructor(
        ventcube: SchwoererVentcube,
        server: string,
        port: number,
        useAdvancedFunctions: boolean,
        interval: number,
    ...
    private reconnect(attempt = 0): void {
        ...
        this.context.log.info(
            `Attempting to reconnect to server. (attempt ${attempt + 1} out of ${this.reconnectAttempts})`,
        );
    ...
    private readFunctionStates(callback: (func: string, value: any, time: Date) => void): void {
        ...
        this.readTimerId = setTimeout(
            function (this: Connector) {
                this.readFunctionStates(callback);
            }.bind(this),
            this.readInterval * 1000,
        );
    }

    public readDataFromHoldingRegister(
        callback: (func: string, value: any, time: Date) => void,
        func: string,
        register: number,
        fields = 1,
    ): any {
        ...
        this.client
            .readHoldingRegisters(register, fields)
                    ...
                    case 'Offline':
                        ...
                        if (!this.isReconnecting) {
                            this.context.log.warn(
                                'Ventcube server seems offline. Data will be retrieved after next successful reconnect.',
                            );
                            this.reconnect();
                        }
                        break;
    ...
    public writeDataToRegister(func: string, register: number, value: number): void {
        ...
        this.client
            .writeMultipleRegisters(register, [s16_buffer.readUInt16BE(0)])
            .then(({ metrics, _request, response }: any) => {
            ...
            .catch((error: Modbus.UserRequestError<any, any>) => {
                switch (error.err) {
                    case 'Offline':
                        ...
                        if (!this.isReconnecting) {
                            this.context.log.warn(
                                'Ventcube server is offline. Please try to update state after successful reconnect again.',
                            );
                            this.reconnect();
                        }
                        break;
            ...
            .catch((error: any) => {
                this.context.log.error(
                    `${error.message}Response: ${error.response.body.message} code: ${error.response.body.code}`,
                );
            });
    }
